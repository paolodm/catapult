...
function authenticate(req, res, next) {
    ...
    */
    ...
    OpenID.authenticate('https://www.google.com/accounts/o8/site-xrds?hd=opower.com',
        (Server.set('env') == 'production' ? 'https://catapult.opower.com' : 'http://localhost:3300') + '/verify',
        null, true, false, function(err, authUrl)
      {
          if (err) {
              log('authenticate error', err.message || err);
              return res.end('Authentication failed: ' + err.message);
...
function verify(req, res, next) {
    ...
    relying_party.verifyAssertion(req, function(err, result){
        if (err) {
