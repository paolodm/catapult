var log =               require('logging').from(__filename),
    Data_Access =       require('./dao/models'),
    Auth =              require('./auth'),
    Constants =         require('./constants'),
    Format =            require('./util/util'),
    Server;
...
function welcome(req, res, next) {
    ...
    var employee = req.session && Data_Access.is_employee(req.session.name);
...
function referrals(req, res, next) {
    var results = {}, query = req.params.q || req.query.q;
    ...
    if (query) {
        ...
        results = Data_Access.referrals(query, req.session);
    } else {
        res.redirect('/referrals/' + Format.url(req.session.name));
    }
    res.render('referrals.ejs', { locals: {
        query:          query,
        page_title:     query,
        results:        results
    } });
...
function sources(req, res, next) {
    ...
    res.render('table.ejs', { locals: {
        page_title:             'Sources',
        collection:             Data_Access.sources(options),
...
function recruiters(req, res, next) {
    ...
    res.render('table.ejs', { locals: {
        ...
        collection:         Data_Access.employees(options),
...
function employee(req, res, next) {
    res.render('swimlanes.ejs', { locals: Data_Access.employee({employee: req.params.employee, session: req.session}) });
}

function job(req, res, next) {
    res.render('swimlanes.ejs', { locals: Data_Access.job({job: req.params.job}) });
}

function team(req, res, next) {
    res.render('swimlanes.ejs', { locals: Data_Access.department({department: req.params.team}) });
}

function update(req, res, next){
    Data_Access.manual_update();
    res.send('updating');
    res.end();
}
...
function addHandlers(options) {
    ...
    Data_Access.init({ auto_update: Server.set('env') == 'production'});
}
