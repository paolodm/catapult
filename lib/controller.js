...
function search(req, res, next) {
    var search_for = Format.unurl(req.params.q || req.query.q);

    res.render('search.ejs', DAL.search({search_for: search_for}));
}


function referrals(req, res, next) {
    var search_for = req.session.name,
        results = DAL.referrals(search_for, req.session);

    res.render('referrals.ejs', {
        page_title:     search_for,
        results:        results
     });
}
...
function sources(req, res, next) {
    ...
    res.render('table.ejs', {
        page_title:             'Sources',
        collection:             false, //DAL.sources(options),
...
function addHandlers(options) {
    ...
    Server.get('/search/:q?', authRequired, search);
    Server.get('/referrals', authRequired, referrals);
    Server.get('/referrals/:q?', authRequired, search);
    ...
    Server.locals({
        page_type:  Constants.PAGE_TYPE.unknown,
        url:        Format.url,
        options:    {},
        has_ticks:  false,
        has_mini:   false,
    ...
    Server.dynamicHelpers({
        ...
        last_update: function() {
            return DAL.last_updated ? Format.minutes(DAL.last_updated) + ' minutes ago' : false;
        },
        next_update: function() {
            return DAL.last_updated ? Format.minutes(+DAL.last_updated + (Constants.UPDATE_INTERVAL_MINUTES * Constants.MILLISECONDS_PER_MINUTE)) * -1 + ' minutes' : false;
        },
        jobvite_down: function() {
            return DAL.jobvite_down;
        }
    });
