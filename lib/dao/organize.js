...
function update(callback) {
    Step(
        ...
        function(err) {
            ...
            if (DB_Candidates.size < 1000 || DB_Employees.size < 4 || DB_Jobs.size < 10) {
                log('It seems jobvite is returning bad data:');
                log('Candidates:', DB_Candidates.size, 'Employees:',  DB_Employees.size, 'Jobs:', DB_Jobs.size );
            }
             else {
            ...
            DB.jobvite_down = !DB.Candidates || !DB.Employees || !DB.Jobs || DB.Candidates.size < 1000 || DB.Employees.size < 4 || DB.Jobs.size < 10;
