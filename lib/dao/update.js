...
function load_jobs(callback) {
    Reports.config({
        ...
        format: function(row) {
            var id = Util.getId(row[0]);
            return id && row.length > 1 ? {
                id: id,
                title: Util.removeHTML(row[0]).trim(),
                department: row[1].trim(),
                recruiters: Util.people(row[2]),
                managers: Util.people(row[3]),
                location: row[4].trim(),
                status: row[5].trim(),
...
function load_candidates (callback) {
    Reports.config({
        ...
        jobvite_password: jobvite_password,
        format: function(row) {
            var id = Util.getId(row[1]);
            if (!id) { return false; }
            var data = {
                id: id,
                date: +new Date(row[0]),
                date_short: row[0],
                name: Util.fix_all_caps(Util.removeHTML(row[1]).trim().replace(/\(.*\)/, '')),
                status: row[2].trim(),
                source_type: row[3].trim().replace(/OPOWER /, ''),
                source: Util.fix_source(row[4]),
                job_id: Util.getId(row[5]),
            ...
            if (data.source) {
                ...
                } else if (data.source_type == 'Friend -- External Referral') {
                ...
                if (data.source_type == 'Employee') {
                    ...
                    data.source_type = 'Employee Referrals';
                }
